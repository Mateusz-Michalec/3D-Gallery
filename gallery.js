...
function changeImg() {
	img.src = `images/${isFullScreen ? 'highres' : 'lowres'}/jelonek-glowa-row-0${row}_${i < 10 ? '00' : '0'}${i}.jpg`
}
...
function mobileZooming(e) {
	...
	if (x > 0 && x < rect.width && y > 0 && y < rect.height) img.style.transformOrigin = `${x}px ${y}px`
	else centerImage()
}
