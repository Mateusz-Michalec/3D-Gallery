...
galleries.forEach((gallery, artworkNumber) => {
	...
	const playAnimationBtn = createGalleryBtn('buttons/play.svg', 'uruchom automatyczne obracanie')
	const fullscreenBtn = createGalleryBtn('buttons/fullscreenon.svg', 'uruchom w pełnym ekranie')
	const closeFullscreenBtn = createGalleryBtn('buttons/fullscreenoff.svg', 'zamknij pełny ekran')
	...
	function loadImage(i) {
		...
		img.src = `/obrotowe/${artwork}/${artwork}-row-0${rows}_${i < 10 ? '00' : '0'}${i}.jpg`
		img.onload = function () {
	...
	function changeImg() {
		img.src = `/obrotowe/${artwork}${highImageQuality ? '/highres' : ''}/${artwork}-row-0${row}_${
