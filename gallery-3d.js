...
window.onload = function () {
	...
	galleries.forEach((gallery, artworkNumber) => {
		...
		function changeImg() {
			...
			console.log(currentImages[currentIndex].currentSrc)
		}

		function setCurrentImages() {
			currentImages = loadedImages.filter(img => img.currentSrc.includes(`row-0${currentRow}`))
			changeImg()
		}
		...
		function getImage(imgNumber, type) {
			return new Promise(resolve => {
				...
				image.src = `./obrotowe/${artwork}/${type === 'highres' ? 'highres/' : ''}${artwork}-row-0${currentRow}_${
					imgNumber < 10 ? '00' : '0'
				}${imgNumber}.jpg`
		...
		function rotateUp() {
			if (currentRow < imagesInRows.length) {
				...
				else if (!isRowLoaded('highres') && !isRowLoaded('lowres')) loadImages('lowres')
		...
		const playAnimationBtn = createGalleryBtn('play', 'uruchom automatyczne obracanie')
		const fullscreenBtn = createGalleryBtn('fullscreenon', 'uruchom w pełnym ekranie')
		const closeFullscreenBtn = createGalleryBtn('fullscreenoff', 'zamknij pełny ekran')
		...
		function updateUI() {
			...
			fullScreenIcon.src = `buttons/${isFullScreen ? 'fullscreenoff' : 'fullscreenon'}.svg`
