...
galleries.forEach((gallery, artworkNumber) => {
  ...
  let imagesInRow = []; // [30,30,30] for instance 3 rows with 30 images in each row
  let errorState = -1;
  let currentlyLoadedRow = 1;
  ...
  function loadImage(imgNumber) {
    ...
    img.src = `/obrotowe/${artwork}/${artwork}-row-0${currentlyLoadedRow}_${
      imgNumber < 10 ? "00" : "0"
    }${imgNumber}.jpg`;

    img.onload = function () {
      imagesInRow[currentlyLoadedRow - 1] = imgNumber;
      loadImage(imgNumber + 1);
    };
  }
  ...
  function rotateLeft() {
    currentIndex--;
    if (currentIndex === 0) currentIndex = imagesInRow[currentRow - 1];
    changeImg();
  }

  function rotateRight() {
    currentIndex++;
    if (currentIndex > imagesInRow[currentRow - 1]) currentIndex = 1;
    changeImg();
  }

  function rotateUp() {
    if (currentRow < imagesInRow.length) {
      currentRow++;
      changeImg();
    } else return;
  }
